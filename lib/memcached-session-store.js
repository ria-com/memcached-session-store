...
//                console.log('GET string parser(data) -->>>',parser(data));
            ...
            if(data.person_id){
                data.passport = {
                    "user":{
                        "user_id": (data.person_info)?data.person_info.email:'',
                        "email":data.person_id,
                        "state_id":(data.person_info)?data.person_info.stateID:0,
                        "city_id":(data.person_info)?data.person_info.cityID:0,
                        "name":data.person_name,
                        "avatar":(data.person_info)?data.person_info.photo:null
                    }
                };
            }
            fn(null, data);
